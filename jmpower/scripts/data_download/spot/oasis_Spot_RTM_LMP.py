...
def generateOASISRequestURL():
	...
	with open("../../../results/gap_2017.txt", 'r') as f:
		line = f.readline()
		...
		while line:
			...
			line = line.replace('\n','')
			line = line.split(',')
			start = datetime.datetime.strptime(line[0],"%Y-%m-%dT%H:%M:%S")
			start = start + datetime.timedelta(minutes=5)
			end = datetime.datetime.strptime(line[1], "%Y-%m-%dT%H:%M:%S")

			diff = end - start
	
			diff = int(diff.seconds/3600) + int(diff.days * 24)

			for i in range(diff):
				startTime = start + datetime.timedelta(hours=i)
...
def storePriceData(report):
	...
	for resourceID in report:
		for interval in report[resourceID]:
			...
			if 'LMP_ENE_PRC' not in dataItem:
				dataItem['LMP_ENE_PRC'] = '0.0'
			if 'LMP_LOSS_PRC' not in dataItem:
				dataItem['LMP_LOSS_PRC'] = '0.0'
			if 'LMP_CONG_PRC' not in dataItem:
				dataItem['LMP_CONG_PRC'] = '0.0'
			if 'LMP_GHG_PRC' not in dataItem:
				dataItem['LMP_GHG_PRC'] = '0.0'
